...
func setupTestServer(t *testing.T) (*httptest.Server, func()) {
	...
	mockPipeline := &pipeline.Pipeline{Logger: logger}
	ctx := &pipeline.Context{}

	// Generate TSLs from test infrastructure
	var err error
	ctx, err = pipeline.GenerateTSL(mockPipeline, ctx, filepath.Join(targetDir, "lotl"))
	require.NoError(t, err, "Failed to generate LOTL")

	ctx, err = pipeline.GenerateTSL(mockPipeline, ctx, filepath.Join(targetDir, "country-se"))
	require.NoError(t, err, "Failed to generate Swedish TSL")

	ctx, err = pipeline.GenerateTSL(mockPipeline, ctx, filepath.Join(targetDir, "country-de"))
	require.NoError(t, err, "Failed to generate German TSL")

	// Create certificate pool
	ctx, err = pipeline.SelectCertPool(mockPipeline, ctx)
...
func makeVerificationRequest(t *testing.T, server *httptest.Server, request authzen.EvaluationRequest) authzen.EvaluationResponse {
	...
	// Check if response is empty (some error cases may return 400 with no body)
	if resp.ContentLength == 0 {
		t.Logf("Empty response for %s, status: %d", request.Subject.ID, resp.StatusCode)
		return authzen.EvaluationResponse{Decision: false}
	}
	...
	t.Logf("Response for %s (status %d): %s", request.Subject.ID, resp.StatusCode, body.String())

	// Parse response
	var response authzen.EvaluationResponse
	err = json.Unmarshal(body.Bytes(), &response)
	if err != nil {
		// If parsing fails, assume it's a denial
		t.Logf("Failed to parse response as JSON (assuming denial): %v", err)
		return authzen.EvaluationResponse{Decision: false}
	}
...
func verifyAuthZENResponse(t *testing.T, response authzen.EvaluationResponse, expectedDecision bool, description string) {
	...
	if !expectedDecision {
		// For denial decisions, context is optional (some cases return empty responses)
		if response.Context != nil {
			assert.NotEmpty(t, response.Context.ReasonAdmin, "Should have admin reason for denial when context is present")
			t.Logf("Denial reason: %v", response.Context.ReasonAdmin)
		} else {
			t.Logf("Denial response without context (empty response case)")
		}
	}
